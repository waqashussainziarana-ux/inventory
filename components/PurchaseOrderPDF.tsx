...
const PurchaseOrderPDF: React.FC<PurchaseOrderPDFProps> = ({ purchaseOrder, products, suppliers }) => {
  ...
  return (
    <div id="po-pdf" className="bg-white p-12 font-sans text-slate-800 mx-auto" style={{ width: '800px', boxSizing: 'border-box', overflow: 'hidden' }}>
      <header className="flex justify-between items-start mb-12 border-b-2 border-slate-900 pb-8">
        <div>
          ...
          <div className="space-y-1">
            <p className="text-slate-500 text-xs font-black uppercase tracking-widest">PO Reference</p>
            <p className="text-xl font-black text-slate-900">{purchaseOrder.poNumber}</p>
          </div>
          <div className="mt-4 space-y-1">
            <p className="text-slate-500 text-xs font-black uppercase tracking-widest">Order Date</p>
      ...
      <section className="mb-12 grid grid-cols-2 gap-10">
        <div>
          ...
          <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 min-h-[120px]">
            <p className="text-2xl font-black text-slate-900">{supplier?.name}</p>
        ...
        <div className="flex flex-col justify-end text-right">
          <div className="space-y-1">
            ...
            <p className="text-xl font-black text-slate-900 uppercase tracking-tighter">{purchaseOrder.status}</p>
          </div>
        </div>
      </section>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-900 text-white">
              <th className="p-5 text-xs font-black uppercase tracking-widest rounded-tl-xl">Product / Category</th>
              <th className="p-5 text-xs font-black uppercase tracking-widest text-center">Qty</th>
              <th className="p-5 text-xs font-black uppercase tracking-widest text-right">Unit Cost</th>
              <th className="p-5 text-xs font-black uppercase tracking-widest text-right rounded-tr-xl">Subtotal</th>
            </tr>
          </thead>
              ...
              <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                <td className="p-5 align-top">
                  <div style={{ breakInside: 'avoid' }}>
                    ...
                    <p className="text-xs font-bold text-indigo-500 uppercase tracking-widest mt-1">{item.category}</p>
                    {imeis.length > 0 && (
                      <div className="mt-3">
                          <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 block mb-1">IMEIs / Serial Numbers:</span>
                          <p className="text-[11px] font-mono text-slate-500 leading-relaxed bg-slate-100/50 p-2 rounded border border-slate-100 whitespace-pre-wrap">
                              {imeis.join(', ')}
                          </p>
                      </div>
                    )}
                ...
                <td className="p-5 text-center font-bold text-slate-700 align-top">{quantity}</td>
                <td className="p-5 text-right font-medium text-slate-600 align-top">{formatCurrency(item.purchasePrice)}</td>
                <td className="p-5 text-right font-black text-slate-900 align-top">{formatCurrency(item.purchasePrice * quantity)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {purchaseOrder.notes && (
        <section className="mb-12" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
             ...
             <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 italic text-slate-600 text-sm leading-relaxed">
                {purchaseOrder.notes}
             </div>
        </section>
      )}

      <footer className="mt-12 flex justify-end" style={{ breakInside: 'avoid' }}>
