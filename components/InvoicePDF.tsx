...
const InvoicePDF: React.FC<InvoicePDFProps> = ({ invoice }) => {
  ...
  return (
    <div id="invoice-pdf" className="bg-white p-12 font-sans text-slate-800 mx-auto" style={{ width: '800px', boxSizing: 'border-box', overflow: 'hidden' }}>
      <header className="flex justify-between items-start mb-12 border-b-2 border-slate-900 pb-8">
        <div>
          ...
          <div className="space-y-1">
            <p className="text-slate-500 text-xs font-black uppercase tracking-widest">Document No.</p>
            <p className="text-xl font-black text-slate-900">{invoice.invoiceNumber}</p>
          </div>
          <div className="mt-4 space-y-1">
            <p className="text-slate-500 text-xs font-black uppercase tracking-widest">Date Issued</p>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-900 text-white">
              <th className="p-5 text-xs font-black uppercase tracking-widest rounded-tl-xl">Item Description</th>
              <th className="p-5 text-xs font-black uppercase tracking-widest text-center">Qty</th>
              <th className="p-5 text-xs font-black uppercase tracking-widest text-right">Unit Price</th>
              <th className="p-5 text-xs font-black uppercase tracking-widest text-right rounded-tr-xl">Total</th>
            </tr>
          </thead>
              ...
              <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'} style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                <td className="p-5 align-top">
                  <div style={{ breakInside: 'avoid' }}>
                    <p className="font-bold text-lg text-slate-900">{group.productName}</p>
                ...
                <td className="p-5 text-center font-bold text-slate-700 align-top">{group.quantity}</td>
                <td className="p-5 text-right font-medium text-slate-600 align-top">{formatCurrency(group.sellingPrice)}</td>
                <td className="p-5 text-right font-black text-slate-900 align-top">{formatCurrency(group.sellingPrice * group.quantity)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mt-32 pt-12 pb-20 border-t border-slate-100 text-center" style={{ breakInside: 'avoid' }}>
